...
import { getProjectContent, getProjectPdf, getProjectScreenshots, getProjectVideo, getProjectYoutubeId } from '../../lib/content';
...
export default async function ProjectPage({ params }) {
  ...
  const youtubeId = getProjectYoutubeId(slug);
    ...
    <LayoutBase>
      <article className="max-w-5xl mx-auto px-8 py-16">
        ...
        <div className="flex flex-wrap gap-2 mb-8">
          {project.status && (
            <span className="text-xs uppercase tracking-wide text-[var(--foreground-muted)] border border-[var(--border)] rounded-full px-2 py-0.5">
              {project.status}
            </span>
          )}
        ...
        {youtubeId && (
          <div className="mb-12">
            ...
            <div className="rounded overflow-hidden border border-[var(--border)] relative" style={{ paddingTop: '56.25%' }}>
              <iframe
